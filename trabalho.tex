...
\usepackage[]
 {hyperref}
\begin{document}
\title{Abordagem probabilistica do trabalho contido}
\maketitle
...
\subsection{Problema da transformação}
...
O termo ``problema de transformação'' pode ainda ser reinterpretado num sentido mais amplo, ou seja, como o problema de elucidar as conexões sistemáticas entre categorias de valor (ou conteúdo de trabalho), por um lado, e categorias de preço, por outro. 

Vamos começar entendendo a teoria de preços de Marx. Vamos partir da ideia de \textbf{preços ideais}, isto é, preços que deveriam prevalecer em um hipotético estado de equilíbrio. Nesta forma de conceber os preços, diz-se que os preços de mercado flutuam em torno desses preços ideais e tendem a convergir para eles. 

Partimos do modelo onde o trabalho contido $\lambda\left(C\right)$ de uma mercadoria $C$ é conectado ao seu preço ideal $\overline{\pi}\left(C\right)$ através da seguinte fórmula:
...
Evidentemente as taxas de lucro $r$ só podem ser uniforme entre as empresas se $K_{n}$ for uniforme, o que não soa realista. Vamos reescrever agora $N\lambda_{0}=\lambda\left(\boldsymbol{V}\right)$ isto é, só para enfatizar que $\boldsymbol{V}$ são as mercadorias consumidas pelos trabalhadores para continuarem vivos, escrevemos então:

\[
r_{G}=\frac{N-\lambda\left(\boldsymbol{V}\right)}{\lambda\left(\boldsymbol{K}_{G}\right)}
\]

Uma vez que $N$ é o número total de trabalhadores, ele é também a quantidade de trabalho (medida em semanas de trabalho) produzido por esses trabalhadores\footnote{Há aqui um ponto importante: a quantidade de trabalho vivo adicionada não corresponde necessariamente ao número de horas fisicamente trabalhadas dentro da firma. Como estamos utilizando a semana como unidade temporal fundamental, uma unidade de força de trabalho corresponde ao aluguel da capacidade de trabalho de um trabalhador durante uma semana inteira. Assim, se um trabalhador é contratado semanalmente para produzir uma cadeira e entrega o produto ao final da semana, computamos que uma unidade semanal de trabalho foi incorporada ao produto, independentemente da distribuição concreta de horas trabalhadas ao longo da semana. Não importa, nesse caso, se o trabalhador trabalhou uma ou oito horas por dia; o que importa é que uma semana de força de trabalho foi mobilizada no processo produtivo. Portanto, se a firma contrata $N$ trabalhadores semanalmente, o processo produtivo incorpora $N$ unidades semanais de trabalho vivo. Estamos trabalhando também com uma abstração na qual os trabalhadores são considerados homogêneos em termos de capacidade produtiva média, se não uma empresa com $n$ trabalhadores poderia incorporar mais ou menos que $n$ horas no trabalho contido, pois seus trabalhadores poderiam ser mais ou menos eficientes que a média da sociedade considerada.} então temos podemos recuperar uma versão análoga da primeira formulação que fizemos de $r$ uma vez que $N=\lambda\left(\boldsymbol{C}_{G}\right)-\lambda\left(\boldsymbol{I}_{G}\right)$

\[
r_{G}=\frac{\lambda\left(\boldsymbol{C}_{G}\right)-\left[\lambda\left(\boldsymbol{I}_{G}\right)+\lambda\left(\boldsymbol{V}\right)\right]}{\lambda\left(\boldsymbol{K}_{G}\right)}
\]

Porém, agora vamos abandonar a ideia de preços ideais e a formulação determinista. Ou seja ao invés de adotarmos $\frac{\overline{\pi}\left(C\right)}{\lambda\left(C\right)}=\psi_{0}$, vamos adotar o preço específico de um conjunto de mercadorias discutido anteriormente:

\begin{align*}
\overline{\Psi}\left(B\right) & =\frac{M\left(B\right)}{L\left(B\right)}=\frac{\pi\left(B\right)}{\lambda\left(B\right)}
\end{align*}

Agora, considerando novamente que as cestas de mercadorias $\left(\boldsymbol{C}_{G},\boldsymbol{I}_{G},\boldsymbol{K}_{G},\boldsymbol{V}\right)$ possuem uma quantidade extremamente grande de mercadorias, e lembrando que o preço específico $\Psi_{j}$ da mercadoria $j$ é modelado como uma variável aleatória obtida a partir de uma distribuição de probabilidade comum a todas as mercadorias,e sendo que o preço específico médio de uma cesta $\boldsymbol{A}$ é $\overline{\Psi}(\boldsymbol{A})=\sum_{j}\frac{\lambda_{j}}{\sum_{k}\lambda_{k}}\Psi_{j}$, onde $\lambda_{j}$ representa o trabalho contido na mercadoria $j$, então como os $\Psi_{j}$ são variáveis aleatórias identicamente distribuídas e as cestas consideradas contêm um número muito grande de mercadorias, segue pela lei dos grandes números que $\overline{\Psi}(\boldsymbol{A})\approx\overline{\Psi}(\boldsymbol{B})$, para quaisquer cestas suficientemente amplas $\boldsymbol{A}$ e $\boldsymbol{B}$.

Então:

\[
r_{G}=\frac{\pi\left(\boldsymbol{C}_{G}\right)-\left[\pi\left(\boldsymbol{I}_{G}\right)+\pi\left(\boldsymbol{V}\right)\right]}{\pi\left(\boldsymbol{K}_{G}\right)}
\]

Porém agora $\pi$ não é mais o preço ideal. Agora por exemplo, se a taxa de lucro da firma $i$ é

\[
r_{i}=\frac{l_{i}}{c_{i}}
\]

Então a média ponderada das taxas de lucros de todo sistema é:

\[
\left\langle r\right\rangle =\frac{\sum_{i}c_{i}r_{i}}{\sum_{j}c_{j}}=\frac{\sum_{i}c_{i}\frac{l_{i}}{c_{i}}}{\sum_{j}c_{j}}=\frac{\sum_{i}l_{i}}{\sum_{j}c_{j}}
\]

E como $r_{G}$ é exatamente a razão entre o lucro e o capital de toda economia então $r_{G}$=$\left\langle r\right\rangle $. Ou seja, abandonamos a hipótese de preços ideais deterministas, segundo a qual toda mercadoria satisfaz exatamente $\frac{\overline{\pi}(C)}{\lambda(C)}=\psi_{0}$, e a substituímos por uma formulação probabilística na qual os preços específicos das mercadorias são variáveis aleatórias. Ainda assim, para agregados suficientemente grandes de mercadorias, a lei dos grandes números implica que as flutuações individuais se compensam, fazendo com que a razão entre preço e trabalho contido para os agregados convirja para um valor médio $\overline{\Psi}$.

\subsubsection{Taxa de mais-valia e queda tangencial da taxa de lucro}

Retomando nossa formulação inicial em termos de preço, vamos definimir a unidade de salario ideal $\overline{\pi}_{0}=1$ como unidade e sendo $\lambda_{0}$ o trabalho contido, então temos pra força de rabalho $\frac{\overline{\pi}_{0}}{\lambda_{0}}=\psi_{0}$. E para qualquer outro cesto de mercadorias então:

\[
\frac{\overline{\pi}\left(C\right)}{\lambda\left(C\right)}=\psi_{0}=\frac{\overline{\pi}_{0}}{\lambda_{0}}=\frac{1}{\lambda_{0}}
\]

Se identificamors a taxa de mais valia como:

\[
e_{M}=\frac{N-\lambda\left(\boldsymbol{V}\right)}{\lambda\left(\boldsymbol{V}\right)}=\frac{N-\lambda_{0}N}{\lambda_{0}N}=\frac{1-\lambda_{0}}{\lambda_{0}}
\]

Isto é, a taxa de mais-valia corresponde à razão entre o trabalho-contido no excedente apropriado pelos capitalistas e o trabalho-contido no conjunto de mercadorias necessárias para reproduzir a força de trabalho.

\[
\lambda_{0}=\frac{1}{\left(1+e_{M}\right)}
\]

Substituindo:

\[
\psi_{0}=\frac{\overline{\pi}\left(C\right)}{\lambda\left(C\right)}=\frac{1}{\lambda_{0}}=\frac{1}{\frac{1}{\left(1+e_{M}\right)}}=1+e_{M}
\]

Conectando a discussão anterior, onde fomos do determinismo para o probabilistico $\psi_{0}\rightarrow\overline{\Psi}$, podemos esperar então que o preço específico médio de um grande agregado de mercadorias convirja para $1+e_{M}$ e é isso que vamos demonstrar agora. 

Pra isso vamos lembrar que $\boldsymbol{V}$ é o agregado das mercadorias consumidas pela população trabahadora por uma semana, e $\pi\left(\boldsymbol{V}\right)$ o preço. Vamos ignorar coisas como poupança ou impostos, etc... ou seja, o trabalhador gasta tudo que recebe, então o salário agregado de todos os trabalhadores na semana é $\pi\left(\boldsymbol{V}\right)$. 

Como escolhemos o salário médio semanal como unidade monetária, o salário agregado pago a $N$ trabalhadores durante uma semana é numericamente igual a $N$.Logo:

\[
\overline{\Psi}(\boldsymbol{V})=\frac{\pi\left(\boldsymbol{V}\right)}{\lambda\left(\boldsymbol{V}\right)}=\frac{N}{\lambda\left(\boldsymbol{V}\right)}=1+e_{M}
\]

Ou seja, para qualquer agregado suficientemente grande de mercadorias $(\boldsymbol{A})$, espera-se que $\overline{\Psi}(\boldsymbol{A})\approx1+e_{M}$. Nesta formulação, a teoria do valor deixa de ser interpretada como uma teoria determinista dos preços individuais e passa a ser entendida como uma regularidade estatística válida apenas no nível agregado.

Além disso se $\lambda\left(\boldsymbol{V}\right)=v$ e $\lambda\left(\boldsymbol{K}_{G}\right)=k$, então como $e_{M}=\frac{N-v}{v}$ e calculando $1+e_{M}=1+\frac{N-v}{v}=\frac{N}{v}$, temos:

\[
r_{G}=\frac{N-v}{k}=\frac{N}{k}\left(\frac{v}{N}\frac{N-v}{v}\right)=\frac{1}{q_{G}}\left(\frac{e_{M}}{1+e_{M}}\right)
\]

Como $\lim_{e_{M}\rightarrow\infty}=\left(\frac{e_{M}}{1+e_{M}}\right)=1$, então se a quantidade de capital por trabalhador $q_{G}=\frac{k}{N}$ tende a crescer $q_{G}\rightarrow\infty$\footnote{Segundo os autores deste livro, essa tendência não se verificou nos dados empíricos. Existem outras interpretações segundo as quais não seria exatamente a taxa geral de lucro que tende a decrescer, mas sim a taxa de lucro incremental (taxa de retorno sobre novos investimentos).}, independente do aumento da taxa de mais valia $e_{M},$ a taxa de lucro tende a diminuir $\lim_{q_{G}\rightarrow\infty}r_{G}=0$. Além disso ainda podemos comentar que podemos relacionar esta quantidade $q_{G}$ com a famosa composição orgânico do capital $c_{0}=\frac{k}{v}$ fazendo, de $e_{M}=\frac{N}{v}-1$ então $v=N/\left(1+e_{M}\right)$então:

\[
c_{0}=\frac{k}{v}=\frac{k}{N}\left(1+e_{M}\right)=\left(1+e_{M}\right)q_{G}\quad\rightarrow q_{G}=\frac{c_{0}}{\left(1+e_{M}\right)}
\]

Ou seja, podemos repetir a discussão anterior citando a composição orgânica do capital:

\[
r_{G}=\frac{e_{M}}{c_{0}}
\]

Novamente, um aumento na composição orgânica do capital (com $e_{M}$ constante) leva a uma queda na taxa de lucro.

\subsubsection{Conclusão}

O principal problema do primeiro modelo parece não residir em sua conexão entre preço e valor, mas sim em seu caráter estritamente determinista. Uma vez reinterpretado probabilisticamente, permitindo que preços individuais flutuem aleatoriamente enquanto regularidades emergem apenas no nível agregado, o modelo torna-se compatível com a dispersão empiricamente observada nos preços e nas taxas de lucro.

Já o modelo apresentado no Volume III preserva o caráter determinista e, além disso, parte da hipótese de uniformidade das taxas de lucro. Essa hipótese é introduzida precisamente porque o modelo determinista original do Volume I não permite obter taxas de lucro uniformes entre setores com diferentes composições de capital. Entretanto, ao tentar impor essa uniformidade, o modelo acaba introduzindo novas dificuldades conceituais e matemáticas.

Enquanto no modelo do Volume I de O Capital há uma preocupação central com a origem do lucro (razão pela qual conceitos como valor e mais-valia ocupam posição fundamental), no modelo do Volume III o foco desloca-se para a questão da distribuição desse lucro. Ainda que ambas as questões possam ser tratadas separadamente, como demonstrado por diversos teóricos, é necessário que exista alguma conexão entre elas, e é precisamente na formulação dessa conexão que surgem várias das polêmicas associadas ao chamado problema da transformação.

É importante ter em mente que abordagens não marxistas, por se interessarem apenas pela distribuição do lucro, podem tratar a teoria do valor-trabalho como dispensável. Já a abordagem marxista, ao buscar também explicar a origem do lucro, necessita ir além da simples distribuição, preservando algum vínculo entre lucro, valor e mais-valia. Dessa forma, ao invés de abandonar o primeiro modelo em favor de um novo sistema determinista baseado em taxas uniformes de lucro, parece mais natural reinterpretar diretamente o modelo original em termos probabilísticos. Nessa abordagem, preços e taxas de lucro individuais podem flutuar livremente, enquanto as regularidades associadas à teoria do valor emergem apenas em nível estatístico e agregado.

Pode-se dizer também que o autor rejeita a hipótese de uniformização das taxas de lucro entre capitais individuais, substituindo-a pela concepção de uma taxa geral de lucro definida como média ponderada das taxas de lucro das empresas, em que os pesos correspondem à magnitude de seus capitais. Em vez de uma taxa de lucro uniforme entre diferentes setores, assume-se, portanto, a existência de uma distribuição relativamente estável de taxas de lucro entre as empresas.
