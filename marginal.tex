...
\usepackage[]
 {hyperref}
\begin{document}
\title{Modelo cinético com abordagem marginalista}
\maketitle
...
\begin{quotation}
\textbf{Observação}: na verdade isso não é apenas uma 'generalização' mas outra classe de modelos que na verdade não modela a troca de riquezas entre agentes, como era o caso anterior, mas modela de certa forma o crescimento de riqueza de uma gente individual. O mercado, diferente do que mencionado, é mais do que apenas um 'intermediário', é uma fonte e sumidora de dinheiro. De fato, temos a mesma motivação pra dizer que os agentes trocam riqueza entre si, do que afirmar que nunca trocam entre si, pois a evolução da riqueza de cada agente acaba sendo, de certa forma, independente.Por fim, a evolução de sua riqueza depende apenas dos próprios parâmetros do agente. Eu decidi restringir a distribuição de um parâmetro ($\alpha_{i}\left(t\right)$) de forma a manter a conservação de dinheiro no sistema e o mercado funcionar efetivamente como um intermediário entre os agentes, mas na argumentação original isso pode ser relaxado, tornando a evolução de cada agente, em grande parte, independente. Essa é uma mudança de modelo que não é explicitado no artigo. Ou seja, especialmente no modelo utilizado para comparação onde no lugar de $\alpha_{i}\left(t\right)$ temos um ruído branco $\xi_{i}\left(t\right)$, e que o artigo busca se aproximar, não é de fato um modelo de interação de agentes, na prática, é apenas o modelo de evolução da riqueza de um agdente.
\end{quotation}
...
Todo modelo é, em certa medida, uma simplificação da realidade; no entanto, o que caracteriza um bom modelo é justamente saber o que deve ser incorporado e o que pode ser abstraído, com base no que se pretende investigar.

\subsubsection*{Modelando a distribuição de riqueza em mercados}
...
Evidentemente, outra possibilidade seria assumir que a introdução de um ruído individualizado por agente ainda preserva qualitativamente os resultados que esperamos observar em “Modeling wealth distribution in growing markets”. Contudo, não encontramos evidências claras disso na literatura consultada, nem no artigo de apoio, nem no artigo original, cujos resultados foram obtidos sob hipóteses distintas.
...
\begin{figure}[h!]
\centering\includegraphics[scale=0.6]{colado13.png}
\end{figure}
