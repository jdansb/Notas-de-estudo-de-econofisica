...
Principais livros lidos, sendo lidos ou a serem lidos: 
\begin{itemize}
\item Presente: 
\begin{itemize}
...
\item ``Capitalism: Competition, Conflict, Crises'' 
...
\end{enumerate}
\item Passado\footnote{Vou inserir apenas aqueles livros que considero mais diretamente relacionados aos meus estudos do doutorado ou que mais me influenciaram na forma de abordar a pesquisa do doutorado.}: 
\begin{itemize}
\item ``Classical Econophysics''\cite{cockshott2012classical} 
\begin{itemize}
\item ``Towards a New Socialism''\cite{cockshott1993towards} 
\end{itemize}
\item ``How Labor Powers the Global Economy''\cite{farjoun2022labor} 
\item ``Laws of Chaos: Probabilistic Approach to Political Economy''\cite{farjoun2020laws} 
\item ``Statistical Laws in Complex Systems''\cite{Altmann_2024}\footnote{Principalmente o capítulo 1 (e talvez o 4) que apresentam a metodologia adotatada que faz uso de leis estatísticas macroscópicas e modelos generativos microscópicos para a investigação de sistemas complexos.} 
\item ``An Introduction to Agent-Based Modeling''\cite{netlogo}\footnote{Principalmente os capítulos 0 e 1 que apresentam o conceito de modelagem baseada em agentes.} 
\item ``O capital no século XXI''\cite{piketty2014capital} 
\item ``Economia política: uma introdução crítica''\cite{netto_braz_economia_politica} 
\begin{itemize}
\item ``O Capital de Marx''\cite{fine_saadfilho_capital_2021} 
\item ``Teoria Econômica Marxista'' \cite{coggiola2024teoria} 
\item ``Curso de Introdução à Economia Política''\cite{singer2000curso} 
\item ``O Capital: compêndio''\cite{cafiero_capital_1981} 
\item ``Iniciação à Teoria Económica Marxista''\cite{mandel1967tratado} 
\end{itemize}
\item ``Historical Materialism'' \cite{historical} 
\begin{itemize}
\item ``The Worldview and Philosophical Methodology of ML'' \cite{worldview2023} 
\item ``A Origem do capitalismo''\cite{wood2001origem} 
\item ``Introdução à filosofia de Marx''\cite{lessa_tonet_introducao_marx} 
\item ``Anti-Duhring''\cite{engels1877antiduhring} 
\item ``Ideologia Alemã''\cite{marx2007ideologia}\footnote{Principalmente a primeira parte. A versão da Expressão Popular no entanto é interessante de ser lido completo.} 
\item ``Ludwig Feuerbach e o fim da filosofia clássica alemã''\cite{engels1886feuerbach} 
\end{itemize}
\end{itemize}
\end{itemize}

\section{Há desigualdade (e isso importa)}
